...
- **A blank, unresponsive window after the machine runs low on memory.** WKWebView reports WebGL context loss whenever its GPU process goes away, and Kimbo answered every loss by building a new context on the next window focus. That call is a synchronous IPC to the GPU process, so when the process is up but unable to answer -- what memory pressure produces, since macOS kills it and the replacement cannot get the resources to start -- the renderer's main thread parks in the kernel and the window paints nothing and takes no input, at 0% CPU, until the GPU process is killed by hand. The platform offers no way to time-box that call, so the first stall cannot be prevented; what is fixed is the repeat. A context creation that blocks the main thread, or fails outright, now retires the GPU renderer for the session instead of re-entering the same wait on every Cmd-Tab.
